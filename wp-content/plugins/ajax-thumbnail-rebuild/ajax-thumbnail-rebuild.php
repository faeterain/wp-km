...
/* Plugin name: AJAX Thumbnail Rebuild
   Plugin URI: https://sajonara.de/2016/03/13/wordpress-plugin-ajaxthumbnailrebuild-deprecated-nachricht-entfernen/
   Author: Alexander Trust
   Author URI: https://sajonara.de
   Version: 1.13
...
class AjaxThRebuild {
	 ...
	 * @return array
	 */
	public static function addRebuildSingle($fields, $post) {
		$thumbnails = array();
	...
	public static function ManagementPage() {
		...
		<?php printf( __("If you find this plugin useful, I'd be happy to read your comments on the %splugin homepage%s. If you experience any problems, feel free to leave a comment too.",
				 'ajax-thumbnail-rebuild'),
			         '<a href="https://sajonara.de/2016/03/13/wordpress-plugin-ajaxthumbnailrebuild-deprecated-nachricht-entfernen/" target="_blank">', '</a>');
		?>
		</p>
...
add_filter( 'attachment_fields_to_edit', array('AjaxThRebuild', 'addRebuildSingle'), 10, 2 );

function addAdminMenu() {
	add_management_page( __( 'Rebuild all Thumbnails', 'ajax-thumbnail-rebuild' ), __( 'Rebuild Thumbnails', 'ajax-thumbnail-rebuild'
), 'manage_options', 'ajax-thumbnail-rebuild', array('AjaxThRebuild', 'ManagementPage') );
}

add_action( 'plugins_loaded', create_function( '', 'global $AjaxThumbnailRebuild; $AjaxThumbnailRebuild = new AjaxThRebuild();' ) );
